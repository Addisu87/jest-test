...
test('mock return value of a function one time', () => {
  ...
  mock('Hello', 'there', 'Addisu'); // call it with 3 different arguments
  expect(mock).toHaveBeenCalledWith('Hello', 'there', 'Addisu');

  mock('Addisu'); // called with 1 argument
  expect(mock).toHaveBeenLastCalledWith('Addisu');
});
...
test('mock implementation of a function', () => {
  const mock = jest.fn().mockImplementation(() => 'Ethiopia');
  expect(mock('Location')).toBe('Ethiopia');
...
// let's match some data against our project
...
test('expect a promise to resolve', async () => {
  const user = {
    getFullName: jest.fn(() => Promise.resolve('Addisu Haile'))
  };
  await expect(user.getFullName('Addisu Haile')).resolves.toBe('Addisu Haile');
});
...
test('expect a promise to reject', async () => {
  ...
  await expect(user.getFullName('Addisu Haile')).rejects.toThrow(
